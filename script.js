...
function renderProductSection(containerId, productList) {
    ...
    container.innerHTML = productList.map(product => `
        <div class="product-card fade-in meal-item" data-id="${product.id}" data-name="${product.name}" data-price="${product.price}">
            ...
            <div class="product-content">
                ...
                <button class="add-button add-to-cart" data-id="${product.id}" data-name="${product.name}" data-price="${product.price}">
...
function confirmOrder() {
    ...
    // VALIDAZIONE NOME OBBLIGATORIO (NUOVO!)
    if (!customerName) {
        showToast('Errore: Inserisci il tuo nome', 'error');
        return;
    }
    ...
    setTimeout(() => {
        processWhatsAppOrder(pickupDate, customerName); // PASSA IL NOME!
        confirmButton.classList.remove('loading');
        spinner.style.display = 'none';
    }, 1500);
}

async function processWhatsAppOrder(pickupDate, customerName) { // RICEVE IL NOME!
    const subtotal = cart.reduce((sum, item) => sum + (item.price * item.quantity), 0);
    const totalQuantity = cart.reduce((sum, item) => sum + item.quantity, 0);
    const discountAmount = (subtotal * appliedDiscount) / 100;
    const finalTotal = subtotal - discountAmount;

    // Prepara dati per Firebase CON NOME REALE
    const orderData = {
        customerName: customerName, // NOME REALE DAL FORM!
        customerPhone: 'Da WhatsApp', // Logico per ordini web
        customerEmail: null,
    ...
    try {
        ...
        console.log('✅ Ordine salvato su Firebase con ID:', docRef.id, 'Nome:', customerName);
        
        // Crea messaggio WhatsApp CON NOME
        ...
        setTimeout(() => {
            clearCart();
            closeCartModal();
            showToast(`✅ Ordine di ${customerName} inviato e salvato con successo!`);
        }, 1000);
...
function clearCart() {
    ...
    // PULISCI ANCHE IL CAMPO NOME (NUOVO!)
    const customerNameField = document.getElementById('customer-name');
